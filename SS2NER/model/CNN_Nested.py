...
def JInter(raw_tensor, scal_ratio):
    b, c, h, w = raw_tensor.shape
    out = torch.zeros((b, c, h * scal_ratio, w * scal_ratio)).to(raw_tensor.device)
    for i in range(scal_ratio):
        for j in range(i + 1):
            out[..., i::scal_ratio, j::scal_ratio] = raw_tensor[..., 0::1, 0::1]
            if i != j:
                out[..., j::scal_ratio, i::scal_ratio] = raw_tensor[..., 0::1, 0::1]
    return out


def JReduce(raw_tensor, scal_ratio):
    batch, channel, high, wide = raw_tensor.shape
    ex = high % scal_ratio
    out = torch.zeros((batch, channel, math.ceil(high / scal_ratio), math.ceil(wide / scal_ratio))).to(raw_tensor.device)
    for n in range(ex):
        for m in range(scal_ratio):
            if m < ex:
                out[:, :, -1, :] = out[:, :, -1, :] + raw_tensor[:, :, high - ex + n, m::scal_ratio]
                out[:, :, :, -1] = out[:, :, :, -1] + raw_tensor[:, :, m::scal_ratio, high - ex + n]
            else:
                out[:, :, -1, :-1] = out[:, :, -1, :-1] + raw_tensor[:, :, high - ex + n, m::scal_ratio]
                out[:, :, :-1, -1] = out[:, :, :-1, -1] + raw_tensor[:, :, m::scal_ratio, high - ex + n]
    if ex != 0:
        out = out / (ex * scal_ratio) * scal_ratio * scal_ratio
        if ex == 1:
            out[:, :, -1, -1] = out[:, :, -1, -1] * (ex * scal_ratio) / 2
        else:
            out[:, :, -1, -1] = out[:, :, -1, -1] * (ex * scal_ratio) / (ex * ex * 2)
    if ex != 0:
        temp = raw_tensor[:, :, :-ex, :-ex]
        for i in range(scal_ratio):
            for j in range(i + 1):
                out[:, :, :-1, :-1] = out[:, :, :-1, :-1] + temp[..., i::scal_ratio, j::scal_ratio]
                if i != j:
                    out[:, :, :-1, :-1] = out[:, :, :-1, :-1] + temp[..., j::scal_ratio, i::scal_ratio]
    else:
        for i in range(scal_ratio):
            for j in range(i + 1):
                out[:, :, :, :] = out[:, :, :, :] + raw_tensor[..., i::scal_ratio, j::scal_ratio]
                if i != j:
                    out[:, :, :, :] = out[:, :, :, :] + raw_tensor[..., j::scal_ratio, i::scal_ratio]
    out = out / (scal_ratio * scal_ratio)

    return out


def JReduce_recover(raw_tensor, scal_ratio, initial_size):
    ex = initial_size % scal_ratio

    batch, channel, high, wide = raw_tensor.shape
    out = torch.zeros((batch, channel, initial_size, initial_size)).to(raw_tensor.device)
    for n in range(ex):
        for m in range(scal_ratio):
            if m < ex:
                out[:, :, initial_size - ex + n, m::scal_ratio] = raw_tensor[:, :, -1, :]
                out[:, :, m::scal_ratio, initial_size - ex + n] = raw_tensor[:, :, :, -1]
            else:
                out[:, :, initial_size - ex + n, m::scal_ratio] = raw_tensor[:, :, -1, :-1]
                out[:, :, m::scal_ratio, initial_size - ex + n] = raw_tensor[:, :, :-1, -1]
    if ex != 0:
        temp = out[:, :, :-ex, :-ex]
        for i in range(scal_ratio):
            for j in range(i + 1):
                temp[..., i::scal_ratio, j::scal_ratio] = raw_tensor[:, :, :-1, :-1]
                if i != j:
                    temp[..., j::scal_ratio, i::scal_ratio] = raw_tensor[:, :, :-1, :-1]
        out[:, :, :-ex, :-ex] = temp
    else:
        for i in range(scal_ratio):
            for j in range(i + 1):
                out[..., i::scal_ratio, j::scal_ratio] = raw_tensor[:, :, :, :]
                if i != j:
                    out[..., j::scal_ratio, i::scal_ratio] = raw_tensor[:, :, :, :]
...
class CNN_Nested(nn.Module):
    ...
    def forward(self, word_reps, cls_embeding, pieces_index, loss_mask):
        ...
        ###############
        # ll_feature,index=nearest_neighbour_l(u_score=u_scores,l_mul=1.5)
        # ll = nearest_neighbour_l_recover(l_feature=ll_feature,index=index,src_weight=u_scores.shape[2],src_height=u_scores.shape[3])
        # ##############
        # ###############
        # ss_feature, index = mean_s(u_score=u_scores, s_mul=0.75)
        # ss = mean_s_recover(s_feature=ss_feature, index=index, src_weight=u_scores.shape[2],
        #                     src_height=u_scores.shape[3])
        # ##############
        scal_ratio = 2

        l_feature = JInter(raw_tensor=u_scores, scal_ratio=scal_ratio)

        m_feature = u_scores

        s_feature = JReduce(raw_tensor=u_scores, scal_ratio=scal_ratio)

        l_feature = F.dropout(l_feature, p=0.1, training=self.training)
        m_feature = F.dropout(m_feature, p=0.1, training=self.training)

        s_feature = F.dropout(s_feature, p=0.1, training=self.training)

        l_mask = seq_len_to_mask(lengths * scal_ratio)  # bsz x length x length
        l_mask = l_mask[:, None] * l_mask.unsqueeze(-1)
        l_pad_mask = l_mask[:, None].eq(0)
        l = self.conv_l_pre_down(l_feature, l_pad_mask)

        l = F.max_pool2d(l, kernel_size=scal_ratio, stride=scal_ratio)

        l = self.conv_l_post_down(l, pad_mask)

        m = self.cnn(m_feature, pad_mask)

        s_mask = seq_len_to_mask(torch.ceil(lengths / scal_ratio))  # bsz x length x length
        s_mask = s_mask[:, None] * s_mask.unsqueeze(-1)
        s_pad_mask = s_mask[:, None].eq(0)
        s = self.conv_s_pre_down(s_feature, s_pad_mask)

        s = JReduce_recover(raw_tensor=s, scal_ratio=scal_ratio, initial_size=u_scores.shape[2])
        s = self.conv_s_post_down(s, pad_mask)

        size_wei = self.cls_mlp(cls_embeding)

        weight = torch.sigmoid(size_wei)
        weight = weight.unsqueeze(2).unsqueeze(3)
        # ttt=weight*s
        lms = (1 - weight) * l + m + weight * s
...
class SS2NER(nn.Module):
    ...
    def forward(self, input_ids, word_ids=None, char_ids=None, token_masks_char=None, char_count=None, word_length=None, pieces_index=None, pieces_length=None,
        ...
        word_embedding, cls_embeding = self.embedding(input_ids, pieces2word, word_ids=word_ids, char_count=char_count, char_ids=char_ids, token_masks_char=token_masks_char)
        w2ner_output = self.CNN_Nested(word_embedding, cls_embeding, pieces_index, loss_mask)
