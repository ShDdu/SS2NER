...
class YoloLayerOutput(nn.Module):
    def __init__(self, config, num_labels, anchor=None):
        super(YoloLayerOutput, self).__init__()
        self.num_labels = num_labels
        self.w2ner_out = config.w2ner_out
        self.yolo_channel = 128
        ...
        self.cnn_2 = nn.Conv2d(in_channels=self.yolo_channel, out_channels=len(config.anchor_size) * (self.num_labels + 5), kernel_size=(1, 1), stride=1, padding=0)
        self.yolo = YoloLayer([(i, i) for i in config.anchor_size] if anchor is not None else anchor, self.num_labels)
...
class SS2NER(nn.Module):
    def __init__(self, config):
        ...
        anchor_m = [(i, i) for i in config.anchor_size]
        anchor_l = [(i - 1, i - 1) for i in config.anchor_size]
        anchor_s = [(i + 2, i + 2) for i in config.anchor_size]
        self.yolo_layer_l = YoloLayerOutput(config, self.num_labels, anchor_l)  # 缩
        self.yolo_layer_m = YoloLayerOutput(config, self.num_labels, anchor_m)  # 原
        self.yolo_layer_s = YoloLayerOutput(config, self.num_labels, anchor_s)  # 放
    ...
    def forward(self, input_ids, word_ids=None, char_ids=None, token_masks_char=None, char_count=None, word_length=None, pieces_index=None, pieces_length=None,
        ...
        yolo_outputs_m, yolo_pred_m = self.yolo_layer_m(w2ner_output)

        l_w2ner_output = JInter(w2ner_output, 2)
        yolo_outputs_l, yolo_pred_l = self.yolo_layer_m(l_w2ner_output)

        s_w2ner_output = JReduce(w2ner_output, 2)
        yolo_outputs_s, yolo_pred_s = self.yolo_layer_m(s_w2ner_output)

        yolo_outputs = torch.cat([yolo_outputs_s, yolo_outputs_l, yolo_outputs_m], dim=1)
        loss = None
        if labels is not None:
            loss_m = self.compute_loss(yolo_pred_m, labels)
            loss_l = self.compute_loss(yolo_pred_l, labels)
            loss_s = self.compute_loss(yolo_pred_s, labels)
            loss = loss_m + loss_l + loss_s
    ...
    def compute_loss(yolo_pred, labels):
        ...
        total_loss = loss_x + loss_y + loss_w + loss_h + loss_conf + 20 * loss_cls  # 总损失
