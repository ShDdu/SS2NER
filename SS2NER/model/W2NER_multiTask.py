...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # ty = torch.zeros((nB, 1), dtype=torch.float).to(pred_boxes.device)
    tw = torch.zeros((nB, nG, nG), dtype=torch.float).to(pred_boxes.device)
    ...
    for s in range(nB):
        ty[s, best_n, :, :] = 1 if len(target[target[:, 0] == s]) > 0 else 0
        start = gx - gw / 2
        end = gx + gw / 2
        for st, en in zip(start, end):
            tw[s, best_n, int(st):int(en), int(st):int(en)] = 1
...
class SS2NER(nn.Module):
    ...
    def compute_loss(yolo_pred, labels):
        ...
        total_loss = loss_x + loss_h + loss_conf + 20 * loss_cls  # 总损失
